...
open_ssh_firewall := {"planned_values": {"root_module": {"resources": [{
...
open_rdp_firewall := {"planned_values": {"root_module": {"resources": [{
	"address": "google_compute_firewall.open_rdp",
	"type":    "google_compute_firewall",
	"values": {
		"direction":    "INGRESS",
		"source_ranges": ["0.0.0.0/0"],
...
restricted_firewall := {"planned_values": {"root_module": {"resources": [{
	"address": "google_compute_firewall.restricted",
	"type":    "google_compute_firewall",
	"values": {
		"direction":    "INGRESS",
...
test_open_ssh_fails if {
	some msg in ac3.deny with input as open_ssh_firewall
	contains(msg, "AC-3")
	contains(msg, "22")
}

test_open_rdp_fails if {
	some msg in ac3.deny with input as open_rdp_firewall
	contains(msg, "AC-3")
	contains(msg, "3389")
}

test_restricted_firewall_passes if {
	count(ac3.deny) == 0 with input as restricted_firewall
}
